...
class Dojo:
    ...
    def __init__(self):
        ...
        self.parameters = {
            'hgt':
            lambda v: re.search(
                "^1(([5-8][0-9])|(9[0-3]))cm|^(59|6[0-9]|7[0-6])in", v),
            'byr':
            lambda v: re.search("^(19[2-9]\d)|^(200[0-2])", v),
            'iyr':
            lambda v: re.search("^20(1\d|20)", v),
            'eyr':
            lambda v: re.search("^20(2\d|30)", v),
            'hcl':
            lambda v: re.search("#([0-9]|[a-f]){6}", v),
            'ecl':
            lambda v: v in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'],
            'pid':
            lambda v: re.search("^[0-9]{9}$", v),
        }
        self.passports = [[]]

    def mount_passport(self, _line):
        if _line == '\n':
            self.passports.append([])
        else:
            self.passports[-1].append(_line)

    def get_keys(self, passport):
        keys = dict()
        for attrs in passport:
            key_values = attrs.split(' ')
    ...
    def run(self, *params):
        ...
        for line in self.input:
            self.mount_passport(line)

        for passport in self.passports:
            key_values = self.get_keys(passport)

            valid = True

            for parameter, func in self.parameters.items():
                value = key_values.get(parameter)
                if not value or not func(value):
                    valid = False
                    break

            # for parameter, func in self.parameters.items():
            #     value = key_values.get(parameter)
            #     if not value and parameter != 'cid':
            #         valid = False
            #         break
